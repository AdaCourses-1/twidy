    ...
    <div>
      ...
      <div className="ml-2 flex-1 flex gap-5 flex-col">
        {SERVICE_ITEMS.map((service: ServiceItem) => (
          ...
          <div>
            <h5 className="text-lg font-bold text-[#4E3F6F]">{service.name}</h5>
            <span className="text-lg font-semibold text-[#8C8CB6]">{service.description}</span>
          </div>
          
          <Button>Заказать за {service.price} {service.currency}</Button>
        </div>
        ))}
